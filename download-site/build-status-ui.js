...
async function fetchTargetBuild() {
  ...
  const response = await fetch(
    `https://api.github.com/repos/${REPOSITORY}/contents/pubspec.yaml?ref=main`,
    {
      cache: "no-store",
  ...
  if (payload?.encoding !== "base64" || typeof payload?.content !== "string") {
    throw new Error("Target build metadata is invalid.");
  }
  const target = parsePubspecVersion(decodeBase64Utf8(payload.content));
  if (!target) throw new Error("Target build version is unavailable.");
  try {
...
function initializeBuildStatusUi() {
  ...
  if (stickyLabel && !stickyLabel.dataset.defaultText) {
    stickyLabel.dataset.defaultText = cleanText(stickyLabel.textContent) || "Current stable HomePilot APK";
  }
  ...
  function enhance() {
    ...
    if (targetBuild) {
      const desiredHeading = `Building HomePilot ${targetBuild.version}`;
