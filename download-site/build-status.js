...
const ALERT_PREFERENCE_KEY = "versiondeck-build-alerts-v1";
...
function estimateRemainingSeconds(steps, currentStep, job, history, now) {
  if (!history?.sampleCount || !Number.isFinite(history.totalMedianSeconds)) {
    return { seconds: null, coverage: 0, confidence: "unavailable" };
  }
  ...
  const confidence = history.sampleCount >= 5 && coverage >= 0.8
    ...
    : history.sampleCount >= 3 && coverage >= 0.5
      ? "medium"
      : "low";
  return { seconds: Math.max(0, seconds), coverage, confidence };
}
...
export function createBuildSnapshot(run, job, history, { now = Date.now() } = {}) {
  ...
  return {
    ...
    status,
    conclusion,
    startedAt: job?.started_at || run.run_started_at || run.created_at || null,
    currentStepName: currentStep?.name || (
      status === "waiting" ? "Waiting for production approval" :
      status === "queued" || status === "requested" || status === "pending"
        ? "Waiting for a GitHub Actions runner"
        : status === "completed"
          ? "Build completed"
...
function remainingPhrase(snapshot) {
  ...
  return `${snapshot.remainingCount} ${snapshot.remainingCount === 1 ? "step remains" : "steps remain"}.`;
}
...
export function detectBuildTransition(previous, current) {
  ...
  if (current.status === "completed" && previous.status !== "completed") {
    const result = current.conclusion === "success" ? "completed successfully" : `finished with ${current.conclusion || "an unknown result"}`;
    return {
      kind: "completed",
      message: `HomePilot production build ${current.runNumber || ""} ${result}.`.replace(/\s+/g, " ").trim(),
    };
  }
  ...
  return {
    kind: "step",
    message: `${completedPhrase} Now running ${current.currentStepName}. ${remainingPhrase(current)} ${estimate}`.replace(/\s+/g, " ").trim(),
  };
}
...
async function fetchGitHubJson(path, { signal } = {}) {
  ...
  try {
    ...
    if (!response.ok) {
      const error = new Error(`GitHub Actions status returned ${response.status}.`);
...
async function fetchRunJob(runId, options) {
  const payload = await fetchGitHubJson(`/actions/runs/${runId}/jobs?filter=latest&per_page=100`, options);
...
function initializeBuildStatus() {
  ...
  const alertsButton = requiredElement("#build-progress-alerts");
  ...
  function setAlertButton() {
    ...
    alertsButton.textContent = alertsEnabled ? "Spoken alerts on" : "Enable spoken alerts";
  }
  ...
  function speak(message) {
    if (!alertsEnabled || !("speechSynthesis" in window) || typeof SpeechSynthesisUtterance === "undefined") return;
  ...
  function notify(message, snapshot) {
    ...
    showToast(message);
    speak(message);
    if (alertsEnabled && document.hidden && "Notification" in window && Notification.permission === "granted") {
      try {
        new Notification("HomePilot production build", {
          body: message,
  ...
  function renderSteps(snapshot) {
    ...
    for (const step of snapshot.steps) {
      ...
      item.className = "build-step-item";
      const state = step.status === "completed"
        ? step.conclusion === "success" ? "completed" : "failed"
  ...
  function render(snapshot) {
    ...
    statusLabel.textContent = snapshot.status === "waiting"
      ? "Approval required"
      : snapshot.status === "queued" || snapshot.status === "requested" || snapshot.status === "pending"
        ? "Queued"
  ...
  alertsButton.addEventListener("click", async () => {
    ...
    if (alertsEnabled) {
      const message = "Spoken HomePilot build alerts enabled.";
      showToast(message);
      speak(message);
    } else {
      window.speechSynthesis?.cancel();
      showToast("Spoken build alerts disabled.");
    }
  });
