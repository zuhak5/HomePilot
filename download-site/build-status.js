...
export function createBuildSnapshot(run, job, history, { now = Date.now() } = {}) {
  ...
  const effectiveStepName = currentStep?.name || failedStep?.name || (
    ...
    QUEUED_STATUSES.has(status) ? "Waiting for a GitHub Actions runner" :
    status === "completed" && conclusion === "success" ? "GitHub Actions completed successfully" :
...
export function detectBuildTransition(previous, current) {
  ...
  if (!previous || previous.runId !== current.runId || previous.runAttempt !== current.runAttempt) {
    return {
      kind: "started",
      message: `HomePilot production build ${current.runNumber || ""} is active. ${current.currentPhaseName}. Current task: ${current.currentStepName}. ${remainingPhrase(current)} ${estimate}`.replace(/\s+/g, " ").trim(),
    };
  }
...
function initializeBuildStatus() {
  ...
  speechButton.addEventListener("click", () => {
    ...
    setSpeechButton();
    if (speechEnabled) {
      const message = "Spoken HomePilot build updates are on.";
      showToast(message, "success");
      speak(message);
    } else {
      window.speechSynthesis?.cancel();
      showToast("Spoken HomePilot build updates are off.", "info");
    }
  });
