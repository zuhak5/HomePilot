export function extractTargetVersion(headingText) {
  const match = String(headingText || "").match(/Building HomePilot\s+(\d+\.\d+\.\d+)/i);
...
function initializeTimelineRefinement() {
  ...
  const defaultLabel = stickyLabel.dataset.defaultText || stickyLabel.textContent.trim() || "Current stable HomePilot APK";
