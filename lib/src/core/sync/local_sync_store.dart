...
''',
  ...
  Future<bool> isDomainDataPristine() async {
    ...
    if (!await _tableExactlyMatchesSeeds(
      'categories',
      _seedValues['category']!,
    )) {
      return false;
    }
  ...
  Future<bool> _outboxContainsOnlySeedUpserts() async {
    final allowed = <String>{
      for (final key in _seedValues['category']!.keys) 'category|$key',
  ...
  Future<void> _seedPristineDefaults() async {
    final now = DateTime.now();
    for (final row in _seedValues['category']!.values) {
...
''',
        variables: [
          ...
          Variable<String>(row['name']! as String),
          Variable<String>(row['health_group']! as String),
          Variable<String>(row['icon_name']! as String),
...
''');
  ...
  Future<int> enqueueMissingDefaultCategories() async {
    final categorySeeds = _seedValues['category']!;
    var inserted = 0;
    for (final entry in categorySeeds.entries) {
      final recordKey = entry.key;
      if (await shadow('category', recordKey) != null) continue;
      final localMatchesSeed = await isUntouchedSeed(
        SyncRecord(
          spec: syncSpecByEntity['category']!,
          recordKey: recordKey,
          values: entry.value,
          clientModifiedAt: DateTime.now().toUtc(),
          originDeviceId: '',
      ...
      if (!localMatchesSeed) continue;
      final changedAt = await db
          .customSelect(
            'SELECT updated_at FROM categories WHERE id = ? LIMIT 1',
            variables: [Variable<String>(recordKey)],
          )
          .getSingleOrNull();
      final updatedAt = changedAt == null
          ? DateTime.now()
          : _sqliteDate(changedAt.data['updated_at']);
...
VALUES ('category', ?, 'upsert', ?, 0)
''',
        variables: [Variable<String>(recordKey), Variable<DateTime>(updatedAt)],
        updates: {db.syncOutbox},
        updateKind: UpdateKind.insert,
      );
      inserted += rows;
    }
    if (inserted > 0) db.markTablesUpdated([db.syncOutbox]);
    return inserted;
  }
...
''',
  ...
  Future<void> applyCatalogCategories(
      ...
      await withOutboxSuppressed(() async {
        for (final row in categories) {
          await _upsertLocal(
            SyncRecord(
              spec: categorySpec,
              recordKey: row['id'] as String,
              values: {
                for (final column in categorySpec.remoteDataColumns)
                  categorySpec.localColumnFor(column): row[column],
              },
...
''',
  ...
  Future<void> markMutationInFlight(
    LocalSyncMutation mutation, {
    required String userId,
  }) async {
  ...
  Future<void> markMutationTerminal(
    LocalSyncMutation mutation,
    String message,
  ) async {
    await (db.update(db.syncOutbox)..where(
...
DateTime _sqliteDate(Object? value) {
