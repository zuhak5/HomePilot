  ...
  wm.Workmanager().executeTask((taskName, inputData) async {
    ...
    try {
      if (taskName == cloudSyncBackgroundTask) {
        return runCloudSyncInBackground(leaseScope: 'work-manager');
      }
      ...
      final scheduler = HomePilotNotificationScheduler(
...
class HomePilotNotificationScheduler implements NotificationScheduler {
  // Public parameter names are clearer for callers while the stored fields stay private.
  HomePilotNotificationScheduler(
  ...
  static const _dueChannelId = 'homepilot_due';
  static const _overdueChannelId = 'homepilot_overdue';
  static const _criticalChannelId = 'homepilot_critical';
  static const _digestChannelId = 'homepilot_digest';
  static const _maintenanceGroupKey = 'homepilot_maintenance';
  ...
  Future<void> initialize() async {
    ...
    if (Platform.isAndroid) {
      await wm.Workmanager().initialize(homeKeeperWorkManagerCallback);
  ...
  Future<void> _scheduleTaskReminder(
    ...
    final body = preferences.privacyMode
        ? l10n.openHomePilotToViewThisReminder
    ...
    await _plugin.zonedSchedule(
      ...
      notificationDetails: NotificationDetails(
        android: AndroidNotificationDetails(
          ...
          groupKey: _maintenanceGroupKey,
          ticker: l10n.homePilotReminderTicker,
  ...
  Future<void> sendTestReminder() async {
    ...
    await _plugin.zonedSchedule(
      ...
      title: l10n.homePilotTestReminder,
  ...
  DateTime _reminderTimeFor(
    ...
    final scheduledDate = task.plan.nextDueDate.subtract(
      Duration(days: task.plan.reminderDaysBefore),
    );
    final hasPlanTime =
        task.plan.nextDueDate.hour != 0 || task.plan.nextDueDate.minute != 0;
    return DateTime(
      scheduledDate.year,
      scheduledDate.month,
      scheduledDate.day,
      hasPlanTime ? task.plan.nextDueDate.hour : preferences.reminderHour,
      hasPlanTime ? task.plan.nextDueDate.minute : 0,
    );
  }
  ...
  DateTime _nextDigestTime(DateTime now, NotificationPreferences preferences) {
    ...
    if (digest.isBefore(now.add(const Duration(minutes: 5)))) {
      digest = digest.add(const Duration(days: 1));
    }
    return digest;
  }
