...
const diagnosticConsentDisclosure =
    'This bundle contains only HomePilot diagnostic events and redacted '
...
class DiagnosticExportService {
  ...
  Future<File> export() async {
    ...
    final directory = Directory(
      p.join(temporary.path, 'homepilot-diagnostics'),
    );
    ...
    final output = File(
      p.join(
        directory.path,
        'homepilot-${createdAt.microsecondsSinceEpoch}-${_uuid.v7()}.zip',
      ),
    );
