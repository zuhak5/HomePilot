...
FutureOr<SentryEvent?> scrubSentryEvent(SentryEvent event, Hint hint) {
  // Sentry's unknown protocol fields cannot be sanitized safely.
  // ignore: invalid_use_of_internal_member
  if (event.unknown?.isNotEmpty ?? false) return null;
  ...
  for (final exception in event.exceptions ?? const <SentryException>[]) {
    exception.value = 'HomePilot ${_safeToken(exception.type ?? 'exception')}';
...
Breadcrumb? scrubSentryBreadcrumb(Breadcrumb? breadcrumb, Hint hint) {
  ...
  if (category == 'homepilot' || category == 'app.lifecycle') {
    return Breadcrumb(
  ...
  return SentryStackTrace(
    ...
    frames: [
      for (final frame in stackTrace.frames)
        SentryStackFrame(
          fileName: _fileNameOnly(frame.fileName),
