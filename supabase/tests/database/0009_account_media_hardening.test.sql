...
select ok(
  not has_table_privilege(
    'authenticated',
    'homepilot_private.account_deletion_cleanup_jobs',
    'SELECT'
  ),
...
select ok(
  not has_function_privilege(
    'authenticated',
    'public.begin_homepilot_account_cleanup(uuid,text[])',
    'EXECUTE'
  ),
...
select ok(
  exists (
    ...
    from pg_constraint
    where conrelid =
        'homepilot_archive.profiles_legacy_media_20260720'::regclass
...
select ok(
  exists (
    ...
    from pg_constraint
    where conrelid =
        'homepilot_archive.asset_photo_upload_metadata_20260720'::regclass
      and confrelid = 'auth.users'::regclass
      and confdeltype = 'c'
  ),
...
select ok(
  exists (
    ...
    from pg_constraint
    where conrelid =
        'homepilot_private.account_deletion_cleanup_jobs'::regclass
      and confrelid = 'auth.users'::regclass
      and confdeltype = 'c'
  ),
...
select ok(
  not has_function_privilege(
    'authenticated',
    'public.is_recent_homepilot_session(uuid,uuid)',
    'EXECUTE'
  ),
...
select ok(
  has_schema_privilege('authenticated', 'homepilot_security', 'USAGE'),
  'authenticated Storage policies can resolve the private helper'
);
select ok(
  not has_schema_privilege('anon', 'homepilot_security', 'USAGE'),
  'anonymous clients cannot resolve privileged policy helpers'
);
select ok(
  to_regprocedure('public.current_homepilot_session_is_active()') is null,
...
select ok(
  to_regclass(
    'homepilot_archive.profiles_legacy_media_user_id_idx'
  ) is not null,
...
select ok(
  to_regclass(
    'homepilot_private.account_deletion_cleanup_jobs_user_id_idx'
  ) is not null,
...
select ok(
  public.is_recent_homepilot_session(
    '55555555-5555-4555-8555-555555555555',
    '66666666-6666-4666-8666-666666666666'
  ),
...
select ok(
  not public.is_recent_homepilot_session(
    '55555555-5555-4555-8555-555555555555',
    '66666666-6666-4666-8666-666666666666'
  ),
...
select is(
  (
    ...
    where schemaname = 'storage'
      ...
      ) like '%current_homepilot_session_is_active%'
  ),
  4,
