...
drop function if exists public.complete_maintenance_task(jsonb, text);

create function public.complete_maintenance_task(
  p_operation jsonb,
  p_device_id text
)
returns jsonb
...
comment on function public.complete_maintenance_task(jsonb, text) is
  'Atomically and idempotently applies one offline maintenance completion.';
