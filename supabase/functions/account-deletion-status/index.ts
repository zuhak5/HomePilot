...
export async function handleAccountDeletionStatus(
  ...
  try {
    const operation = await services.lookupOperation(
      requestHash,
      subjectBinding,
    );
    if (operation == null) {
  ...
  return {
    async lookupOperation(requestHash, subjectBinding) {
      const { data, error } = await admin.rpc(
        "lookup_homepilot_account_deletion_operation",
        {
...
async function readRecoveryPayload(
  request: Request,
): Promise<{ recoveryKey: string; expectedUserId: string } | null> {
  try {
    ...
    return { recoveryKey, expectedUserId };
  } catch {
    return null;
  }
}
