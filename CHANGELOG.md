...
- **Hosted Advisor remediation**: Added an explicit service-role-only policy for private deletion-recovery operations, removed eleven hosted-statistics-confirmed obsolete or redundant indexes, and retained the two low-traffic archive indexes required for Auth-user cascade cleanup without granting client access.
