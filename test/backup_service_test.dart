...
import 'package:homepilot/src/core/services/backup_service.dart';
...
void main() {
  ...
  group('ZipBackupService', () {
    ...
    setUp(() async {
      root = await Directory.systemTemp.createTemp('homepilot_backup_test_');
    ...
    test(
      ...
      () async {
        ...
        expect(p.basename(backupPath), startsWith('homepilot-backup-'));
    ...
    test('restores backup state without duplicating current data', () async {
      ...
      expect(
        File(
          p.join(docs.path, 'photos', assets.single.id, 'seed-photo.jpg'),
        ).existsSync(),
        isTrue,
      );
      expect(
      ...
      expect(
        ...
        startsWith('homepilot-before-restore-'),
      );
...
Future<void> _seedRealisticData(AppDatabase db, Directory root) async {
  ...
  File(
    p.join(legacyProfileDir.path, 'avatar.jpg'),
...
Future<File> _createFormatOneBackup(Directory root) async {
  final dbFile = File(p.join(root.path, 'old-homepilot.sqlite'));
  ...
  final backup = File(p.join(root.path, 'old-homepilot.zip'));
