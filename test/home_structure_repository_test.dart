...
import 'package:homepilot/src/core/services/weather_service.dart';
      ...
      await db
          ...
          .insert(
            SettingsCompanion.insert(
              ...
              value:
                  '{"enabled":true,"localReminders":true,"maxRemindersPerDay":4}',
...
''',
  ...
  test('database reopen preserves streak state', () async {
    final root = await Directory.systemTemp.createTemp('homepilot_reopen_');
  ...
  test('migrates schema 1 rooms and infers thing types', () async {
    final db = AppDatabase(executor: NativeDatabase.memory(setup: _createV1));
    ...
    expect(
      rooms.singleWhere((room) => room.name == 'General').areaId,
      'area_first_floor',
    );
    expect(
      rooms.singleWhere((room) => room.name == 'Kitchen').areaId,
      'area_first_floor',
    );
    expect(
      rooms.singleWhere((room) => room.name == 'Garden').areaId,
      'area_outdoor_garden',
    );
    expect(
      (await repo.listAreas())
          .singleWhere((area) => area.id == 'area_outdoor_garden')
          .name,
      'Outdoor',
    );
    expect(
      assets.singleWhere((asset) => asset.name == 'Air conditioner').assetType,
      AssetType.device,
    );
    expect(
      assets.singleWhere((asset) => asset.name == 'Water bowl').assetType,
      AssetType.pet,
    );
  });
}
...
void _createV1(dynamic database) {
...
''');
  database.execute(
    ...
    "('room_general', 'General', 0, 0), "
    "('room_kitchen', 'Kitchen', 0, 0), "
    "('room_garden', 'Garden', 0, 0)",
  );
  database.execute(
    "INSERT INTO categories(id, name, health_group, icon_name, created_at, updated_at) VALUES "
    "('category_appliances', 'Appliances', 'appliances', 'kitchen', 0, 0), "
    "('category_pets', 'Pets', 'pets', 'pets', 0, 0)",
  );
  database.execute(
    "INSERT INTO assets(id, name, category_id, room_id, notes, created_at, updated_at) VALUES "
    "('asset_ac', 'Air conditioner', 'category_appliances', 'room_kitchen', 'north wall', 0, 0), "
    "('asset_bowl', 'Water bowl', 'category_pets', 'room_garden', 'shade corner', 0, 0)",
  );
