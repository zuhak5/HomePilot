...
void main() {
  group('Failed Mutation Diagnostics & Resolution (P0-B, P1-B)', () {
    ...
    test(
      'exportFailedMutationDiagnostics returns privacy-safe fields without raw user payload',
      () async {
    ...
    test(
      'resolveFailedMutation dismisses only the selected row and leaves others intact',
      () async {
        final now = DateTime.now().toUtc();
        await db
            .into(db.syncOutbox)
