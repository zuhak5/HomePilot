...
void main() {
  ...
  test(
    ...
    () async {
      ...
      await store.applyCatalogCategories([
        {
          'id': 'category_general',
      ...
      final category = await (db.select(
        db.categories,
      )..where((row) => row.id.equals('category_general'))).getSingle();
      expect(category.name, 'General updated');
      expect(await store.pendingCount(), 0);
    },
  );
  ...
  test('untouched defaults are recognized only while unchanged', () async {
    ...
    final categoryRecord = SyncRecord(
      spec: syncSpecByEntity['category']!,
      recordKey: 'category_general',
      values: {
        'id': 'category_general',
        'name': 'General',
        'health_group': 'other',
    ...
    expect(await store.isUntouchedSeed(categoryRecord), isTrue);
    ...
    await (db.update(db.categories)
          ..where((row) => row.id.equals('category_general')))
        .write(const CategoriesCompanion(name: Value('General local')));

    expect(await store.isUntouchedSeed(themeRecord), isFalse);
    expect(await store.isUntouchedSeed(categoryRecord), isFalse);
  });
  ...
  test('fresh-install detection rejects edited default category', () async {
    expect(await store.isPristineForCloudBootstrap(), isTrue);

    await (db.update(db.categories)
          ..where((row) => row.id.equals('category_general')))
        .write(const CategoriesCompanion(name: Value('Everything else')));
