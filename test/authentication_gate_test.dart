...
import 'package:homepilot/src/features/auth/presentation/authentication_gate.dart';
...
void main() {
  ...
  testWidgets('language selector is compact without losing tap targets', (
    ...
    final visibleSelector = find.byKey(
      const ValueKey('onboarding-language-selector'),
    );
    ...
    final visibleSize = tester.getSize(visibleSelector);
    expect(visibleSize.width, lessThanOrEqualTo(180));
    expect(visibleSize.height, lessThanOrEqualTo(36));

    final hitSize = tester.getSize(hitTarget);
    expect(hitSize.height, greaterThanOrEqualTo(48));

    for (final key in [
      const ValueKey('language-segment-en-hit-target'),
      const ValueKey('language-segment-ar-hit-target'),
    ]) {
  ...
  testWidgets('language selector switches immediately in LTR and RTL', (
    ...
    await tester.tap(
      find.byKey(const ValueKey('language-segment-ar-hit-target')),
    );
    await tester.pump();
    ...
    await tester.tap(
      find.byKey(const ValueKey('language-segment-en-hit-target')),
    );
    await tester.pump();
  ...
  testWidgets('shell stays locked until Google authentication completes', (
    ...
    expect(find.text('HomePilot app'), findsNothing);
  ...
  testWidgets('Continue with Google creates the only cloud session', (
    ...
    expect(find.text('HomePilot app'), findsOneWidget);
  });
  ...
  testWidgets('first launch exposes autonym language control only', (
    tester,
  ) async {
    final auth = _FakeAuthRepository();
    addTearDown(auth.dispose);
      ...
      home: hk_ui.HkMotion(
        ...
        child: AuthenticationGate(
          language: locale.languageCode == 'ar'
              ...
              : AppLanguage.en,
          onLanguageChanged: (_) {},
          child: const Text('HomePilot app'),
        ),
      ),
    ),
  );
