...
import 'package:homepilot/src/core/sync/sync_connectivity.dart';
import 'package:homepilot/src/core/sync/sync_contracts.dart';
...
void main() {
  ...
  testWidgets('startup failures honor an Arabic device locale', (tester) async {
    ...
    await tester.pumpWidget(
      const HomePilotStartupFailure(cloudUnavailable: true),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets('initial hydration blocks Home until restoration completes', (
    ...
    expect(find.text('Finalizing HomePilot'), findsOneWidget);
  ...
  testWidgets('finalization failure names the finalizing step', (tester) async {
    ...
    expect(find.text('Finalizing HomePilot needs attention'), findsOneWidget);
    expect(find.text('Cloud data is still waiting'), findsNothing);
    expect(
      find.text(
        'HomePilot could not complete Finalizing HomePilot. Retry to continue.',
      ),
      findsOneWidget,
    );
    expect(find.text('Continue offline'), findsNothing);
  ...
  testWidgets('verified Home snapshot enables offline recovery and tip', (
    ...
    final container = ProviderScope.containerOf(
      tester.element(find.byType(HomePilotApp)),
    );
    final failedEnable = Completer<void>();
  ...
  testWidgets('dashboard shell renders when onboarding is complete', (
    ...
    expect(find.textContaining('Thulfiqar.'), findsOneWidget);
  ...
  testWidgets('shared floating action button keeps fixed dimensions', (
    ...
    await tester.pumpWidget(
      MaterialApp(
        theme: testLightTheme(),
        home: Scaffold(
          floatingActionButton: hk_ui.HomePilotFloatingActionButton(
            icon: Icons.add,
            label: 'Create',
            onPressed: () {},
          ),
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      for (final theme in [HomePilotTheme.light(), HomePilotTheme.dark()]) {
        for (final width in [320.0, 600.0]) {
          ...
          expect(
            tester.getSize(points).height,
            hk_ui.kHomePilotHeaderActionHeight,
          );
          expect(
          ...
          final pointsMaterial = tester.widget<Material>(
            find.descendant(of: points, matching: find.byType(Material)).first,
          );
          ...
          expect(notificationMaterial.color, pointsMaterial.color);
          expect(notificationMaterial.shape, pointsMaterial.shape);
          expect(tester.takeException(), isNull);
        }
      }
      semantics.dispose();
    ...
    final repository = FakeBackupRepository(
      state: BackupState(
        lastBackup: BackupStatus(
          ...
          path: 'C:\\backups\\homepilot-backup.zip',
    ...
    (tester) async {
      final repository = FakeBackupRepository(
        exportCompleter: Completer<String>()
          ..complete('C:\\backups\\homepilot-backup.zip'),
      );
      ...
      expect(find.text('Creating backup...'), findsNothing);
      expect(find.byType(CircularProgressIndicator), findsNothing);
      expect(find.textContaining('homepilot-backup.zip'), findsOneWidget);
      expect(tester.takeException(), isNull);
    },
  );
  ...
  testWidgets('backup button shows inline loading after the delay', (
    ...
    completer.complete('C:\\backups\\homepilot-backup.zip');
    await tester.pumpAndSettle();
  ...
  testWidgets('backup screen previews a selected zip and opens restore dialog', (
    tester,
  ) async {
    final repository = FakeBackupRepository(
    ...
    expect(find.text('Restore locally and pause cloud backup'), findsOneWidget);
    expect(
...
List<Override> _testOverrides(
  ...
  return [
    ...
    assetRepositoryProvider.overrideWithValue(
      assetRepository ??
          StartupAssetRepository(assets: things, rooms: _rooms(now)),
    ),
    ...
    roomStream == null
        ? roomsProvider.overrideWithValue(AsyncData(_rooms(now)))
        : roomsProvider.overrideWith(
            (ref) => _seededStream(_rooms(now), roomStream),
          ),
