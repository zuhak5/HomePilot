...
The manually dispatched Play rail builds and verifies one signed production AAB, captures manifest/dependency/signature evidence, uploads retained Actions artifacts, and records an exact provenance tuple covering repository, source digest/ref, signer workflow identity/digest, workflow trigger, runner environment, and workflow run URI. It contains no Google Play API upload or rollout step. Play Console acceptance, App Signing, declarations, and device delivery require the separately authorized operator process in the [Google Play release runbook](../operations/google-play-release-runbook.md).

The separately dispatched APK rail validates source and production configuration, builds and inspects one signed non-debuggable APK, captures evidence, publishes the Sentry release, records the same exact provenance tuple, and creates the GitHub Release. Those mutations occur only in the protected workflow; source inspection does not establish that a run succeeded.

VersionDeck is separate from the application build. It discovers releases and independently verifies APK identity plus the exact provenance tuple before generating a static download site. The manifest carries an explicit publication state and an absolute trust-lease expiry; downloads fail closed when publication is disabled, the lease expires, or a reviewed historical disposition marks a release withdrawn. Live workflow status is informational and must remain separate from verified stable-release identity.
