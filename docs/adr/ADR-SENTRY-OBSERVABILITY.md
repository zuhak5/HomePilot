...
HomePilot needs crash and performance diagnostics across Flutter, Android, synchronization, authentication, backup, monetization, and release behavior. The application also handles sensitive household content, identity/session information, media, location-dependent features, and backup files.
...
- Associate production events with a protected CI-published release.

Production release mutation is performed by the protected Android workflow rather than ordinary local development commands.
...
Rejected because it may collect more context than is necessary and compatible with HomePilot's privacy goals.
