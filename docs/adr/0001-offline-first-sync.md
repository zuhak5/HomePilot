...
HomePilot manages information users need during ordinary household work, including when connectivity is unavailable or unstable. Requiring a network round trip for every asset or maintenance interaction would degrade reliability. At the same time, authenticated users expect recovery and multi-device synchronization.
