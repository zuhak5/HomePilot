...
The workflow also requires a `release_attempt_id` shaped like `hpra_<32 lowercase hex characters>` and the corresponding `release_attempt_run_id` alongside the exact current `main` SHA, project ref, and `apply-pending-migrations` confirmation. The pre-environment job restores `HomePilot-release-attempt-dry-run-<attempt_id>`, verifies that it is `prerequisites_verified` for the exact repository/source SHA with `backend_validation` evidence, and reruns semantic prerequisites: Google release contracts, Edge Function formatting/type checks/tests, Supabase lint, and local database tests.

Inside the protected `production-supabase-migrations` job, the workflow revalidates exact current `main`, project identity, and the dry-run ledger again after approval, runs a linked dry run, and verifies the ledger immediately before `npx supabase db push --linked`. It records a `mutation_started` ledger entry before the apply call, a `published` entry after the apply call returns successfully, and an `externally_verified` entry only after a follow-up dry run shows no pending migrations. If the apply starts but later verification fails, treat the hosted database as partial external state and reconcile with the same release attempt instead of starting a fresh attempt silently.

This binds hosted migration mutation to the release attempt ledger described in the [release attempt ledger runbook](../operations/release-attempt-ledger.md). A syntactically valid attempt ID is not, by itself, hosted deployment approval or proof that the backend aggregate passed; retain the dry-run ledger evidence, migration ledger artifact, protected workflow approvals, and no-pending verification in the release record.

Record migration identifiers, function versions/source commits, local test results, hosted target, deployment operator, compatibility assumptions, mutation-start/apply/no-pending ledger states, and any required mobile release ordering.
