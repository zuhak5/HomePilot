...
HomePilot's intended Sentry configuration must preserve all of the following unless a separately approved privacy decision changes policy:
...
2. Sanitize exception text and breadcrumbs.
3. Remove URL query strings and signed object paths where present.
4. Drop request bodies and unsafe headers.
5. Remove user objects and direct identifiers.
6. Bound collection sizes.
7. Fail closed when an unknown rich payload cannot be proven safe.
...
Do not copy sensitive event content into GitHub issues or pull requests.
